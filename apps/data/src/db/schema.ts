...
export const lesson = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("lesson_one_published_per_chapter")
      .on(table.chapter)
...
export const lessonText = pgTable(
  ...
  (table) => [
    ...
    check(
      "lesson_text_title_non_blank",
      sql`length(btrim(${table.title})) > 0`,
    ),
    check(
      "lesson_text_content_non_blank",
...
export const mediaAsset = pgTable(
  ...
  (table) => [
    ...
    check(
      "media_asset_duration_ms_positive",
      sql`${table.durationMs} is null or ${table.durationMs} > 0`,
    ),
    check(
      "media_asset_content_type_non_blank",
...
export const lessonAudio = pgTable(
  "lesson_audio",
  {
    ...
    lessonId: integer("lesson_id")
      ...
      .references(
        () => lesson.id,
        { onDelete: "cascade" }, // if the child row is deleted, parent will deleted aswell
      ),
    languageCode: varchar("language_code", { length: 35 })
      .notNull()
      .references(() => language.code, { onDelete: "restrict" }),
...
export const schemaRelations = defineRelations(schema, (relations) => ({
  ...
  lessonAudio: {
    ...
    lessonText: relations.one.lessonText({
      from: [
        relations.lessonAudio.lessonId,
        relations.lessonAudio.languageCode,
      ],
